...
{
    const unsigned long long id = threadIdx.x + blockIdx.x * blockDim.x;
    __shared__ int blockSum = 0;
    // TODO: Your code here
    if (id < N)
        atomicAdd(&blockSum, input[id]);
    __syncthreads()
    if (threadIdx.x == 0)
    {
        atomicAdd(&dResult, blockSum);
    }
}
...
__global__ void reduceShared(const unsigned long long* __restrict input, unsigned long long N)
{
    ...
    */
    ...
    while(step < BLOCK_SIZE){
        int window = step * 2;
        int prevPoolIdx = ((int)(threadIdx.x / step)*step);
        int poolIdx = ((int)(threadIdx.x / window) * window);
        if (prevPoolIdx != poolIdx && prevPoolIdx == threadIdx.x){
                //printf("poolIdx for %d at step %d:%d \n", threadIdx.x, step, poolIdx);
                //printf("prevPoolIdx for %d at step %d: %d \n", threadIdx.x, step, prevPoolIdx);

                atomicAdd(&(data[poolIdx]),data[prevPoolIdx]);
        }
        __syncthreads();
        if (threadIdx.x == 0) {
                step *= 2;
